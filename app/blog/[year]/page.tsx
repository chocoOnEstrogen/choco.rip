...
export async function generateMetadata({ params }: YearPageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Blog posts from the year ${year}`,
    openGraph: {
      title: `Blog Posts from ${year}`,
      description: `Blog posts from the year ${year}`,
      type: "website",
      url: `${config.url}/blog/${year}`,
    },
    twitter: {
    ...
    alternates: {
      canonical: `${config.url}/blog/${year}`,
    },
  }
}
...
export default async function YearPage({ params }: YearPageProps) {
  ...
  const { posts, metadata } = await getBlogPosts(year)
      ...
      <div className="flex flex-col gap-12">
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight">Blog Posts from {year}</h1>
            <p className="text-muted-foreground max-w-2xl text-lg">
              All blog posts published in {year}
            </p>
          </div>
