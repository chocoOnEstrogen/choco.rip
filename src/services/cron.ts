...
type CronSchedule = `${number | '*'} ${number | '*'} ${number | '*'} ${number | '*'} ${number | '*'}`;
...
class CronService {
    ...
    registerJob(job: CronJob | readonly CronJob[]): void {
        ...
        jobs.forEach(j => {
            ...
            const scheduledJob = cron.schedule(j.schedule, async () => {
                if (j.enabled === false) return;

                try {
                    const timeoutPromise = j.timeout 
                        ? new Promise((_, reject) => setTimeout(() => reject(new Error('Job timed out')), j.timeout))
                        : null;
