...
async function setupConfig() {
    ...
    config.app.name = await question(rl, 'Application name', `${username}'s Website`)
    ...
    config.seo.twitter_handle = await question(rl, 'Twitter handle', `@${username}`)
    ...
    const setupDiscord = await question(rl, 'Do you want to set up Discord? (y/N)')
    ...
    // Footer Links
    console.log(chalk.yellow('\nFooter Links:'))
    const setupFooter = await question(rl, 'Do you want to set up footer links? (y/N)')
